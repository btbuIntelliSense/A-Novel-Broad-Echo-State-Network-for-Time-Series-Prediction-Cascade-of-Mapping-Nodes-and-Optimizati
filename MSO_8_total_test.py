...
if __name__ == '__main__':
    ...
    for i in range(10, 40, 2):
        for j in range(2 ,30, 2):
            ...
            BLS_predictTrain, BLS_predictTest, BLS_TRAIN_TIME, BLS_TEST_TIME = BLS(traindata, trainlabel, testdata, testlabel,s=0.8, c=2**-28, N1=10, N2=i, N3=j)
            CFBLS_predictTrain, CFBLS_predictTest, CFBLS_TRAIN_TIME, CFBLS_TEST_TIME = CFBLS(traindata, trainlabel, testdata, testlabel, s=0.8, c=2 ** -28, N1=10, N2=i,N3=j)
            LCFBLS_predictTrain, LCFBLS_predictTest, LCFBLS_TRAIN_TIME, LCFBLS_TEST_TIME = LCFBLS(traindata, trainlabel, testdata, testlabel, s=0.8, c=2 ** -28, N1=10,N2=i,N3=j)
            CEBLS_predictTrain, CEBLS_predictTest, CEBLS_TRAIN_TIME, CEBLS_TEST_TIME = CEBLS(traindata, trainlabel, testdata, testlabel, s=0.8, c=2 ** -28, N1=10,N2=i,N3=j)
            LCFBLS_ESN_predictTrain, LCFBLS_ESN_predictTest, LCFBLS_ESN_TRAIN_TIME,LCFBLS_ESN_TEST_TIME = LCFBLS_ESN(traindata, trainlabel, testdata, testlabel, s=0.8, c=2 ** -28, N1=10,N2=i,N3=j)
            ...
            # 计算R
            # print(BLS_predictTrain.shape,trainlabel.shape)
            ...
            print(
                "LCFBLS_ESN_TrainRMSE : {}, LCFBLS_ESN_TrainR2 : {}".format(LCFBLS_ESN_TrainRMSE, LCFBLS_ESN_TrainR2))
            print("LCFBLS_ESN_TestRMSE : {}, LCFBLS_ESN_TestR2 : {}".format(LCFBLS_ESN_TestRMSE, LCFBLS_ESN_TestR2))

            #训练集的数据保存
            pd.DataFrame(BLS_predictTrain.reshape(-1,1)).to_csv("E:\\yan_2\\CFBLS_LCFBLS复现\\result\\csvFile\\MSO_8\\BLS\\train\\BLS_mapping_{}_enhance_{}.csv".format(i, j), index=False)
            pd.DataFrame(CFBLS_predictTrain.reshape(-1,1)).to_csv(
                "E:\\yan_2\\CFBLS_LCFBLS复现\\result\\csvFile\\MSO_8\\CFBLS\\train\\CFBLS_mapping_{}_enhance_{}.csv".format(i, j),
                index=False)
            pd.DataFrame(LCFBLS_predictTrain.reshape(-1,1)).to_csv(
                "E:\\yan_2\\CFBLS_LCFBLS复现\\result\\csvFile\\MSO_8\\LCFBLS\\train\\LCFBLS_mapping_{}_enhance_{}.csv".format(i, j),
                index=False)
            pd.DataFrame(CEBLS_predictTrain.reshape(-1,1)).to_csv(
                "E:\\yan_2\\CFBLS_LCFBLS复现\\result\\csvFile\\MSO_8\\CEBLS\\train\\CEBLS_mapping_{}_enhance_{}.csv".format(i, j),
                index=False)

            pd.DataFrame(BLS_ESN_predictTrain.reshape(-1,1)).to_csv(
                "E:\\yan_2\\CFBLS_LCFBLS复现\\result\\csvFile\\MSO_8\\BLS_ESN\\train\\BLS_ESN_mapping_{}_enhance_{}.csv".format(i, j),
                index=False)
            pd.DataFrame(CFBLS_ESN_predictTrain.reshape(-1,1)).to_csv(
                "E:\\yan_2\\CFBLS_LCFBLS复现\\result\\csvFile\\MSO_8\\CFBLS_ESN\\train\\CFBLS_ESN_mapping_{}_enhance_{}.csv".format(i, j),
                index=False)
            pd.DataFrame(LCFBLS_ESN_predictTrain.reshape(-1,1)).to_csv(
                "E:\\yan_2\\CFBLS_LCFBLS复现\\result\\csvFile\\MSO_8\\LCFBLS_ESN\\train\\LCFBLS_ESN_mapping_{}_enhance_{}.csv".format(i, j),
                index=False)

            # 测试集的数据保存
            pd.DataFrame(BLS_predictTest.reshape(-1,1)).to_csv(
                "E:\\yan_2\\CFBLS_LCFBLS复现\\result\\csvFile\\MSO_8\\BLS\\test\\BLS_mapping_{}_enhance_{}.csv".format(i, j),
                index=False)
            pd.DataFrame(CFBLS_predictTest.reshape(-1,1)).to_csv(
                "E:\\yan_2\\CFBLS_LCFBLS复现\\result\\csvFile\\MSO_8\\CFBLS\\test\\CFBLS_mapping_{}_enhance_{}.csv".format(i, j),
                index=False)
            pd.DataFrame(LCFBLS_predictTest.reshape(-1,1)).to_csv(
                "E:\\yan_2\\CFBLS_LCFBLS复现\\result\\csvFile\\MSO_8\\LCFBLS\\test\\LCFBLS_mapping_{}_enhance_{}.csv".format(
                    i, j),
                index=False)
            pd.DataFrame(CEBLS_predictTest.reshape(-1,1)).to_csv(
                "E:\\yan_2\\CFBLS_LCFBLS复现\\result\\csvFile\\MSO_8\\CEBLS\\test\\CEBLS_mapping_{}_enhance_{}.csv".format(i, j),
                index=False)

            pd.DataFrame(BLS_ESN_predictTest.reshape(-1,1)).to_csv(
                "E:\\yan_2\\CFBLS_LCFBLS复现\\result\\csvFile\\MSO_8\\BLS_ESN\\test\\BLS_ESN_mapping_{}_enhance_{}.csv".format(
                    i, j),
                index=False)
            pd.DataFrame(CFBLS_ESN_predictTest.reshape(-1,1)).to_csv(
                "E:\\yan_2\\CFBLS_LCFBLS复现\\result\\csvFile\\MSO_8\\CFBLS_ESN\\test\\CFBLS_ESN_mapping_{}_enhance_{}.csv".format(
                    i, j),
                index=False)
            pd.DataFrame(LCFBLS_ESN_predictTest.reshape(-1,1)).to_csv(
                "E:\\yan_2\\CFBLS_LCFBLS复现\\result\\csvFile\\MSO_8\\LCFBLS_ESN\\test\\LCFBLS_ESN_mapping_{}_enhance_{}.csv".format(
                    i, j),
                index=False)
    ...
    BLS_Train_datafram = {'mappingNumber': mappingNumberList.reshape(-1, ),
                          ...
                          'SMAPE': BLS_SMAPE_TRAIN.reshape(-1, ), 'R2': BLS_R2_TRAIN.reshape(-1, ),
                           'time': BLS_TIME_TRAIN.reshape(-1,),}
    pd.DataFrame(BLS_Train_datafram).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\BLS\\MSO_8\\train\\BLS_train.csv', index=False)
    ...
    BLS_Test_datafram = {'mappingNumber': mappingNumberList.reshape(-1, ),
                          ...
                          'SMAPE': BLS_SMAPE_TEST.reshape(-1, ), 'R2': BLS_R2_TEST.reshape(-1, ),
                           'time': BLS_TIME_TEST.reshape(-1,),}
    pd.DataFrame(BLS_Test_datafram).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\BLS\\MSO_8\\test\\BLS_test.csv', index=False)
    ...
    CFBLS_Train_datafram = {'mappingNumber': mappingNumberList.reshape(-1, ),
                          ...
                          'time': CFBLS_TIME_TRAIN.reshape(-1,), }
    pd.DataFrame(CFBLS_Train_datafram).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\CFBLS\\MSO_8\\train\\CFBLS_train.csv', index=False)
    ...
    CFBLS_Test_datafram = {'mappingNumber': mappingNumberList.reshape(-1, ),
                         ...
                         'time': CFBLS_TIME_TEST.reshape(-1,),}
    pd.DataFrame(CFBLS_Test_datafram).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\CFBLS\\MSO_8\\test\\CFBLS_test.csv', index=False)
    ...
    LCFBLS_Train_datafram = {'mappingNumber': mappingNumberList.reshape(-1, ),
                            ...
                            'SMAPE': LCFBLS_SMAPE_TRAIN.reshape(-1, ), 'R2': LCFBLS_R2_TRAIN.reshape(-1, ),
                             'time': LCFBLS_TIME_TRAIN.reshape(-1,),}
    pd.DataFrame(LCFBLS_Train_datafram).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\LCFBLS\\MSO_8\\train\\LCFBLS_train.csv', index=False)
    ...
    LCFBLS_Test_datafram = {'mappingNumber': mappingNumberList.reshape(-1, ),
                           ...
                           'SMAPE': LCFBLS_SMAPE_TEST.reshape(-1, ), 'R2': LCFBLS_R2_TEST.reshape(-1, ),
                            'time': LCFBLS_TIME_TEST.reshape(-1,),}
    pd.DataFrame(LCFBLS_Test_datafram).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\LCFBLS\\MSO_8\\test\\LCFBLS_test.csv', index=False)
    ...
    CEBLS_Train_datafram = {'mappingNumber': mappingNumberList.reshape(-1, ),
                             ...
                             'SMAPE': CEBLS_SMAPE_TRAIN.reshape(-1, ), 'R2': CEBLS_R2_TRAIN.reshape(-1, ),
                              'time': CEBLS_TIME_TRAIN.reshape(-1,),}
    pd.DataFrame(CEBLS_Train_datafram).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\CEBLS\\MSO_8\\train\\CEBLS_train.csv', index=False)
    ...
    CEBLS_Test_datafram = {'mappingNumber': mappingNumberList.reshape(-1, ),
                            ...
                            'SMAPE': CEBLS_SMAPE_TEST.reshape(-1, ), 'R2': CEBLS_R2_TEST.reshape(-1, ),
                             'time': CEBLS_TIME_TEST.reshape(-1,),}
    pd.DataFrame(CEBLS_Test_datafram).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\CEBLS\\MSO_8\\test\\CEBLS_test.csv', index=False)
    ...
    BLS_ESN_Train_datafram = {'mappingNumber': mappingNumberList.reshape(-1, ),
                             ...
                             'SMAPE': BLS_ESN_SMAPE_TRAIN.reshape(-1, ), 'R2': BLS_ESN_R2_TRAIN.reshape(-1, ),
                              'time': BLS_ESN_TIME_TRAIN.reshape(-1,),}
    pd.DataFrame(BLS_ESN_Train_datafram).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\BLS_ESN\\MSO_8\\train\\BLS_ESN_train.csv', index=False)
    ...
    BLS_ESN_Test_datafram = {'mappingNumber': mappingNumberList.reshape(-1, ),
                            ...
                            'SMAPE': BLS_ESN_SMAPE_TEST.reshape(-1, ), 'R2': BLS_ESN_R2_TEST.reshape(-1, ),
                             'time': BLS_ESN_TIME_TEST.reshape(-1,),}
    pd.DataFrame(BLS_ESN_Test_datafram).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\BLS_ESN\\MSO_8\\test\\BLS_ESN_test.csv', index=False)
    ...
    CFBLS_ESN_Train_datafram = {'mappingNumber': mappingNumberList.reshape(-1, ),
                              ...
                              'SMAPE': CFBLS_ESN_SMAPE_TRAIN.reshape(-1, ), 'R2': CFBLS_ESN_R2_TRAIN.reshape(-1, ),
                               'time': CFBLS_ESN_TIME_TRAIN.reshape(-1,),}
    pd.DataFrame(CFBLS_ESN_Train_datafram).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\CFBLS_ESN\\MSO_8\\train\\CFBLS_ESN_train.csv', index=False)
    ...
    CFBLS_ESN_Test_datafram = {'mappingNumber': mappingNumberList.reshape(-1, ),
                             ...
                             'time': CFBLS_ESN_TIME_TEST.reshape(-1,),}
    pd.DataFrame(CFBLS_ESN_Test_datafram).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\CFBLS_ESN\\MSO_8\\test\\CFBLS_ESN_test.csv', index=False)
    ...
    LCFBLS_ESN_Train_datafram = {'mappingNumber': mappingNumberList.reshape(-1, ),
                                ...
                                'time': LCFBLS_ESN_TIME_TRAIN.reshape(-1,),}
    pd.DataFrame(LCFBLS_ESN_Train_datafram).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\LCFBLS_ESN\\MSO_8\\train\\LCFBLS_ESN_train.csv', index=False)
    ...
    LCFBLS_ESN_Test_datafram = {'mappingNumber': mappingNumberList.reshape(-1, ),
                               ...
                               'SMAPE': LCFBLS_ESN_SMAPE_TEST.reshape(-1, ), 'R2': LCFBLS_ESN_R2_TEST.reshape(-1, ),
                                'time':LCFBLS_ESN_TIME_TEST.reshape(-1,),}
    pd.DataFrame(LCFBLS_ESN_Test_datafram).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\LCFBLS_ESN\\MSO_8\\test\\LCFBLS_ESN_test.csv', index=False)
    ...
    pd.DataFrame(dataframTest).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\huizong\\MSO_8\\huizong_test.csv', index=False)
    ...
    pd.DataFrame(dataframTrain).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\huizong\\MSO_8\\huizong_train.csv', index=False)
    ...
    pd.DataFrame(dataframTrain_RMSE_TIME).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\huizong\\MSO_8\\huizong_train_RMSE_TIME.csv', index=False)
    ...
    pd.DataFrame(dataframTest_RMSE_TIME).to_csv(
        'E:\\yan_2\\CFBLS_LCFBLS复现\\result\\zhibiao\\huizong\\MSO_8\\huizong_test_RMSE_TIME.csv', index=False)
